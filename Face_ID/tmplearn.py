...
"""
GOOGLE_CLOUD_UTIL_INSTALL =>  https://cloud.google.com/storage/docs/gsutil_install#mac
...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"]="./key/My First Project-d91696a35625.json"

DESTINATION_BUCKET = "smartcaller_bucket"
TRAINING_FILE = "gs://"+DESTINATION_BUCKET+"/training.csv"
VALIDATION_FILE = "gs://"+DESTINATION_BUCKET+"/validation.csv"
...
FLAGS.local_image_dir=FLAGS.base_dir+'input_images/'
...
def gcs_copy(source, dest):
    print('Recursively copying from %s to %s' %(source, dest))
    subprocess.check_call(['gsutil', '-q', '-m', 'cp', '-R'] + [source] + [dest])
...
def main(argv=None):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
        ...
        gcs_copy(model_dir, 'gs://smartcaller_bucket/')
...
def prepare_data():
    ...
    gcs_copy( TRAINING_FILE,'.')
    gcs_copy( VALIDATION_FILE,'.')
